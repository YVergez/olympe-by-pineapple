...
(*fonction pricipale*)
let do_all carteori cartelig pas (*listequadra*) =
  let t_ref =
  ...
  let listequadra =
    [(0,0,255,0);(0,63,0,20);(0,127,0,40);(0,191,0,60);(0,255,0,80);(0,255,127,100);(127,255,127,120);(191,255,191,140)] in
    print_string("ok !");
    print_newline ();
  let listriangle =
    ref (recuptriangles mori mlig pas (t_ref.height) (t_ref.width)
	   listequadra) in
    print_string("ok triangle reussit !");
    print_newline ();
    writeobj "mypointaha.obj" listriangle;;

let main () =
  do_all "carte3d.bmp" "carte-edged.bmp" 20;
  exit 0;;
